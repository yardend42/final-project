...
//edit vacation
const editVacation = async (vacationId: number, updatedVacation: Vacation) => {
  const formattedStartDate = updatedVacation.start_date.toString().slice(0, 10);
  const formattedEndDate = updatedVacation.end_date.toString().slice(0, 10);
  ...
  const sql = `
        ...
        SET destination = '${updatedVacation.destination}', 
            ...
            image_filename = '${updatedVacation.image_filename}'
